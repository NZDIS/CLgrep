...
int main(int argc, char * argv[]) {
    ...
    char *kernelFile = "./kernel/SH_CPU_Kernel.cl";//The kernel file's name
    ...
    err |= clSetKernelArg(kernel, 5, sizeof(needleNum), &needleNum);
    err |= clSetKernelArg(kernel, 6, sizeof(int)*needleNum, NULL);//localCounter
    err |= clSetKernelArg(kernel, 7, sizeof(cl_mem), &lastPosOfEachNeedleBuffer);
    err |= clSetKernelArg(kernel, 8, sizeof(cl_mem), &lenOfEachNeedleBuffer);
    err |= clSetKernelArg(kernel, 9, sizeof(cl_mem), &resBuffer);
    if(err != CL_SUCCESS) {
