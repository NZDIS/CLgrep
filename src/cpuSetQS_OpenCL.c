 ...
 * 		It is based on CPU with OpenCL.
...
//Print the result for atoc_inc method in the kernel
void PrintTheSimpleResults(int *res, string *needles, int needleNum) {
    int i=0;
    while(i<needleNum) {
        printf("matched %s =  %d \n", needles[i], res[i]);
        i++;
...
int main(int argc, char * argv[]) {
    ...
    char *kernelFile = "./kernel/SQ_CPU_Kernel.cl";//The kernel file's name
    ...
    size_t localSize;//Global Size and Local Size
    size_t globalSize;
    localSize = 64;
    globalSize = 64*4;//CPU
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(longestNeedleLen), &longestNeedleLen);
    err |= clSetKernelArg(kernel, 4, sizeof(charsPerItem), &charsPerItem);
    err |= clSetKernelArg(kernel, 5, sizeof(int), &needleNum);
    err |= clSetKernelArg(kernel, 6, sizeof(int)*needleNum, NULL);//localCounter
    err |= clSetKernelArg(kernel, 7, sizeof(cl_mem), &lenOfEachNeedleBuffer);
    ...
    //~ PrintTheSimpleResults(res, needles, needleNum);
