...
__kernel void SetSundayMatch(
	...
	int charsPerItem,
	int needleNum,
	__local int *localCounter,//This still here
    ...
    while(j<=charsPerItem) {//How about the last work_item, the charsPerItem already minus the shortest needles length!!!(note)
        ...
        for(k=0; k<needleNum; k++) {
			int scan = 0;
            int lenOfThisNeedle = lenOfEachNeedle[k];
            while(scan < lenOfThisNeedle && localNeedlesData[scan+pass]==haystack[offset+scan+j]) {
                scan++;
            }
            if(scan == lenOfThisNeedle) {
                 privateCounter[k]++; 
            }
            int jump = tempTable[haystack[offset+j+lenOfThisNeedle]];
            if(jump >= 1 && shift > jump){
				shift = jump;
			}
			pass = pass+lenOfThisNeedle;  
        }
        j+= shift;
    }
    

    //Send pri to local group counter
